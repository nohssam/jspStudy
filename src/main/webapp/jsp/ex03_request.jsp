...
<%--
    ...
    ** request 생명주기 는 response(응답)하면 끝난다.
    ...
    request가 가지고 있는 정보들
	  ...
	  ** 2. 파라미터의 정보 제공
	   ...
	   ** 다른 주요 메서드
	   - 포워드 : request.getrequestDispacher("이동할주소").forward(reqeust,response);
	   - 속성을 이욯한 데이터 저장 : request.setAttribute("이름", 데이터);
	   - 속성에 저장된 데이터 호출 : request.getAttribute("이름") 또는 ${이름} => EL
	   - 모델에서 나중에 세션을 호출할때 사용 : request.getSession();
 --%>
     <h2>Request를 활용한 클리언트 정보 및 서버 정보 보기</h2>
     <h3>
     	<ul>
     		...
     		<li>요청 정보 정보 전달 발식 : <%= request.getMethod()%>  </li>
